    ...
    public function render($request, Throwable $exception)
    {
        if ($this->isHttpException($exception) && $exception->getStatusCode() == 404) {
            ...
            $language = Cache::remember("language_{$lang}", $cacheDuration, fn () =>
                DB::table("language")->where('status', 1)->orderBy('order_seq', 'asc')->get()
            );

            $navCategories = Cache::remember("navcategories_{$lang}", $cacheDuration, fn () =>
                DB::table('categories_has_main_pro as chmp')
            ...
            $categoriesByMain = collect([1, 2, 3, 4])->mapWithKeys(function ($id) use ($lang, $cacheDuration) {
                ...
                return [$id => Cache::remember($cacheKey, $cacheDuration, fn () =>
                    DB::table('categories_has_main_pro as chmp')
                        ->join('sub_pro_categories as sc', 'chmp.cate_id', '=', 'sc.sub_pro_id')
                        ->join('sub_pro_categories_translation as sct', 'sct.sub_pro_id', '=', 'sc.sub_pro_id')
                        ->select('sc.*', 'sct.*', 'chmp.*')
            ...
            $navApplication = Cache::remember("navapplication_{$lang}", $cacheDuration, fn () =>
                DB::table('application as ap')
            ...
            $navAboutUs = Cache::remember("navaboutus_{$lang}", $cacheDuration, fn () =>
                DB::table('about_us as au')
            ...
            $countryEmails = Cache::remember("countryemails_{$lang}", $cacheDuration, fn () =>
            ...
            $mailChimpCountry = Cache::remember("mail_chimp_country_{$lang}", $cacheDuration, fn () =>
                DB::table('mail_chimp_country')->orderBy('name', 'asc')->get()
            );

            $staticWordCache = Cache::remember("static_word_{$lang}", $cacheDuration, fn () =>
                DB::table('static_keyword as w')
            ...
            if ($staticWordCache->isEmpty()) {
                $staticWordCache = Cache::remember("static_word_en", $cacheDuration, fn () =>
                    DB::table('static_keyword as w')
                        ->join('static_keyword_translations as skt', 'skt.key_word', '=', 'w.key_word')
                        ->select('w.*', 'skt.*')
                        ->where('skt.local', 'en')

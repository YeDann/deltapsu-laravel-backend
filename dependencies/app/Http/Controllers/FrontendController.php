...
class FrontendController extends Controller
{
    ...
    public function index($page = 'home')
    {
        ...
        if ('success-case' == $page) {
            ...
            $news_type = DB::table('success_case_type as nt')
            ->join('success_case_type_translation as ntt', 'ntt.fk_sct_id', '=', 'nt.id')
            ->select('nt.*', 'ntt.title as typename')
            ->where('ntt.local', $lang)
            ->orderBy('nt.order_seq', 'asc')
            ->get();
            ...
            if (isset($_GET['type-id']) && 0 != $_GET['type-id']) {
                $type_id = $this->validateInput($_GET['type-id'], 'number', true);
                $news = DB::table('product_success_case_has_categories as pnc')
                ->join('contents as c', 'c.id', '=', 'pnc.content_id')
                ->join('contents_translations as ct', 'ct.content_id', '=', 'c.id')
            ...
            return view('front-end.success-case')
            ->with('metatag', $metatag)
            ->with('news_type', $news_type)
            ->with('status_eol', $status)
            ->with('type_id', $type_id)
        ...
        if ('sales-offices' == $page) {
            return redirect()->route('contactSalesOffices');
        }
        if ('distributors' == $page) {
    ...
    public function allproductsByType($cate_parname, $cate_par_id = 0, $main_pId)
    {
        ...
        if ($cate && $cate->url_item !== $cate_parname && 3 != $main_pId) {
    ...
    public function removeDuplicates($array, $propertyName)
    {
        $uniqueValues = [];
        $resultArray = [];
    ...
    public function appDetailById($app_name, $app_id)
    {
        ...
        if (null != $application && $percent > 80) {
    ...
    public function updateSuccessCaseDetail($namePar)
    {
        // Get the current URL
        $currentUrl = url()->current();
        $lowercaseUrl = strtolower($currentUrl);
        ...
        if (0 != count($contents)) {
            $otherNews = DB::table('product_success_case_has_categories as pnc')
            ->join('contents as c', 'c.id', '=', 'pnc.content_id')
            ->join('contents_translations as ct', 'ct.content_id', '=', 'c.id')
            ->join('success_case_type as nt', 'nt.id', '=', 'pnc.categories_id')
            ->join('success_case_type_translation as ntt', 'ntt.fk_sct_id', '=', 'nt.id')
    ...
    public function confighistory(Request $request)
    {
        ...
        if (array_key_exists('sort', $params)) {
            if ('asc' == $params['sort'] || 'desc' == $params['sort']) {
                $order = $params['sort'];
            }
        }
    ...
    public function searchAll($keySearchQuery)
    {
        ...
        $query = DB::table('products as p')
                    ...
                    ->where('st.local', $lang)
                    ->where('p.enable_pro', 1)
                    ->where(function ($q) use ($cleanQueryString) {
    ...
    public function SubmitContact(Request $request)
    {
        ...
        if ($body->success) {
            ...
            //Enquiry Pdf
            if (null != $request->config_id && 0 == $request->enquireStatus) {
                ...
                //Send Pdf to me
            } elseif (null != $request->config_id && 3 == $request->enquireStatus) {
                $data = DB::table('configuration_history')->where('id', $request->config_id)->get();
                if (isset($data) && count($data) > 0) {
                    DB::table('configuration_history')->where('id', $data[0]->id)->update(
                        [
    ...
    public function checkpartnerAccount(Request $request)
    {
        ...
        if ('' != $members) {
            ...
            $user = DB::table('partner')->where('id', '=', $members->id)->update([
              'pin' => $pin,
              ]);
            session(['pin' => $pin]);
            $link = $members->id;
    ...
    public function downloadFIle($typefilePar, $modelPar)
    {
        ...
        if ('Test_Report' == $typefile && !isset($partnerId)) {
            return redirect()->route('index', 'login');
        }
        ...
        if (file_exists($path) && isset($file) && null != $file && '' != $file) {
            ...
            if ('stp' == $pathinfo['extension'] || 'dxf' == $pathinfo['extension']) {
    ...
    public function loadparallercon(Request $request)
    {
    ...
    public function searchValue($fil_id, $proid, $type, $unit, $propertys)
    {
        $data_result = '';
        // return dd($propertys);
        if ($proid) {
            if ('number' == $type) {
                ...
                foreach ($propertys as $item) {
                    if ($item->type_id == $fil_id && $item->product_id == $proid) {
                        if (null != $item->value_text && 'null' != $item->value_text) {
                            $raw2 = str_replace('<br>', "\n", $item->value_text);
    ...
    public function checkNullData($dataarr, $unit, $status)
    {
        $stringText = '-';
        $arrstri = [];
    ...
    public function getDimansion($product)
    {
        // return dd($product);
        $str = '';
    ...
    public function getUnitWeight($product)
    {
        $sum = 0;
        $String = '';
        if ($product) {
    ...
    public function partnerLoginDoc(Request $request)
    {
        ...
        if (0 != count($partner)) {
            ...
            if ($checkPas) {
                ...
                $margeting = null;
                if (2 == $partner[0]->role || 1 == $partner[0]->role) {
                    $margeting = DB::table('marketing_resource as mr')
                            ->join('marketing_resource_translations as mrt', 'mr.id', '=', 'mrt.mr_id')
                            ->join('marketing_resource_cate as mc', 'mc.cate_id', '=', 'mr.cate_id')
                            ->join('permission_marketcate as permar', 'permar.market_cate_id', '=', 'mc.cate_id')
    ...
    private function getDataNew($query, $type)
    {
        ...
        foreach ($query as $item) {
            $data_date = '';
            if ('events' == $type) {
                $date = self::getDateformat($item->date_publish ?? '00:00:00');
                $endDate = self::getDateformat($item->date_end ?? '00:00:00');
                $data_date = $date['m'] . ' ' . $date['d'] . '' . (isset($endDate['d']) ? ' - ' . $endDate['d'] : '') . ' ' . $date['y'];
                $color_type = '';
                $cateName = '';
            } elseif ('news' == $type) {
                $datenew = self::getDateformat($item->date_info ?? '00:00:00');

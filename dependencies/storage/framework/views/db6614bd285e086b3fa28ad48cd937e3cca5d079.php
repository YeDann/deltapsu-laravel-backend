		...
		<div id="configurable">
			...
			<section> 
				...
				<div class="row add-space-mobile reverse-on-mobile ">
					...
					<div class="col-lg-6 col-md-12" style="padding-left:24px;">
	...
	function setValueSlot(_this){
		...
		if(amp >= 800){
			...
			if(!checkSlotMax(2)){
				...
				parent.children('.col-3').children('b').html('<?php echo e($staticContent['Slot']); ?>'+_index+' - '+(_index+2));
			...
			if(parent.children('input[name=slot]').val() == 3){
				parent.children('.col-3').children('b').html('<?php echo e($staticContent['Slot']); ?> '+_index);
				$.each($('.slot .child-slot'),function(index,value){
					if($(value).children('input[name=index]').val() > _index){
						this_val = parseInt($(value).children('input[name=index]').val())-2;
						$(value).prop('id','child-'+this_val);
						$(value).children('input[name=index]').val(this_val);
						this_index = ($(value).children('input[name=slot]').val() != 1) ? this_val+' - '+(this_val+2) : this_val;
						$(value).children('.col-3').children('b').html('<?php echo e($staticContent['Slot']); ?> '+this_index);
						$(value).children('.select-box').children('.col-3:last-child').children('.btn-undo-icon').attr('onclick','resetData('+this_val+')');
					}
				});
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.front-end', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/Deltapsu_Production/dependencies/resources/views/front-end/configurableproduct.blade.php ENDPATH**/ ?>
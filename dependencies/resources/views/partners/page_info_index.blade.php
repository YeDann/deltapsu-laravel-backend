    ...
    <div class="block block-rounded block-bordered">
        ...
        <div class="block-content block-content-full">
            <table class="table table-bordered table-striped table-vcenter js-dataTable-full">
                ...
                <tbody>
                    @if(isset($static_content) and !empty($static_content))
                    @foreach ($static_content as $item)
                    <td class="text-center">{{$loop->iteration}}</td>
                    <td class="d-none d-sm-table-cell">{{$item->title}}</td>
                    <td class="text-center">
                        <a href="{{route('page_info_edit', [$item->id])}}" class="btn btn-primary">Edit</a>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="block block-themed block-transparent mb-0">
                    ...
                    <form action="{{route('deteletepartpage')}}" method="POST" >
                        {{csrf_field()}}

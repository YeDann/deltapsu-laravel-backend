    ...
    <div class="content content-full">
        <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
            ...
            <nav class="flex-sm-00-auto ml-sm-3" aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"> <a href="{{route('launch_datail' ,$headId)}}">Edit Product Launch
                            Detail</a></li>
                                ...
                                <div class="tab-pane {{($loop->iteration == 1)?" active":""}}"
                                    ...
                                    <div class="">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" name="modelname[{{$item->name}}]"
                                                placeholder="Enter text..."
                                                value="{{isset($current->modelname)?$current->modelname:''}}" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="example-select">op_voltage </label>
                                            <input type="text" class="form-control" name="op_voltage[{{$item->name}}]"
                                                placeholder="Enter text..."
                                                value="{{isset($current->op_voltage)?$current->op_voltage:''}}"
                                                required>
                                        </div>
                                        <div class="form-group">
                                            <label for="example-select">op_wattage </label>
                                            <input type="text" class="form-control" name="op_wattage[{{$item->name}}]"
                                                placeholder="Enter text..."
                                                value="{{isset($current->op_wattage)?$current->op_wattage:''}}"
                                                required>
                                        </div>
                                        <div class="form-group">
                                            <label for="example-select">Phase </label>
                                            <input type="text" class="form-control" name="phase[{{$item->name}}]"
                                                placeholder="Enter text..."
                                                value="{{isset($current->phase)?$current->phase:''}}" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="example-select">Remark </label>
                                            <input type="text" class="form-control" name="remark[{{$item->name}}]"
                                                placeholder="Enter text..."
                                                value="{{isset($current->remark)?$current->remark:''}}" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="example-select"> Old File</label>
                                            <a
                                                href="{{config('app.url')}}/file_doc_2/marketing_resources/{{isset($current->file) ? $current->file :''}}">{{isset($current->file)
                                                ? $current->file :''}}</a>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="custom-file " style="width:100%;">
                                                <input type="file" class="custom-file-input"
                                                    name="filepro[{{$item->name}}]" data-toggle="custom-file-input">

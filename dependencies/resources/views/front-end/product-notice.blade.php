...
<style>
    ...
    #select-product-notice option {
        text-transform: capitalize;
    }

    #select-product-notice {
        text-transform: capitalize;
    }
...
<section class="box-news">
    <div class="container">
        ...
        <select id="select-product-notice" data-fn-change="selectDataProductNotice" class="form-control invisible-up-922 mb-4 w-75 m-auto border-radius-6">
            ...
            <div class="col-md-12 ">
                <div class="nav nav-tabs d-flex justify-content-center border-b-2px visible-up-922 mb-5" id="nav-tab"
                    ...
                    <a class="nav-item nav-link font-size-tab  {{$type_id == 0 ? 'active' :''}}"
                        href="{{route('index', ['page' => 'product-notice'])}}?type=all&type-id=0">{{$staticContent['All']}}</a>
                    ...
                    <a class="nav-item nav-link font-size-tab position-relative {{$type_id == $type->id ? 'active' :''}}"
                        href="{{route('index', ['page' => 'product-notice'])}}?type={{preg_replace('/\s+/', '-',strtolower($type->typename))}}&type-id={{$type->id}}">{{$type->typename}}
                        @if((
...
<script>
    ...
    function selectDataProductNotice(){
        var value_tab = document.getElementById("select-product-notice").value;
        var type_name = '';
        if(value_tab != 0){
            ...
            if(find){
             ...
             type_id =  value_tab;
             window.location = '{{route('index', ['page' => 'product-notice'])}}?type='+type_name +'&type-id='+type_id ;
            }
        }else{
            window.location = "{{route('index', ['page' => 'product-notice'])}}?type=all&type-id=0";
        }
     }
</script>

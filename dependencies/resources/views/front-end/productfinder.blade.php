...
<div class="visible-tablets-up">
            ...
            <div class="container">
                <nav aria-label="breadcrumb" id="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active text-breadcrumb" aria-current="page"><a href="#">{{$staticContent['Product_Selector']}}</a></li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <div class="box-product-finder mb-5">
        <div class="container">
            <h2 class="text-title-delta">{{$staticContent['Product_Selector']}}</h2>
            <div class="row">
                @foreach($subCategories as $sub)
                    <div class="col-lg-4 col-md-6 mb-4">
                        <div class="border-r">
                        <a class=""  href="{{ route('producsList',[preg_replace('/\s+/', '_', $sub->name),$sub->sub_pro_id])}}">
                        @if($sub->image != null)
                        <img class="w-100 mt-2" src="{{config('app.url')}}/medias/categories/{{$sub->image}}" alt="">
                        @else 
...
<div class="visible-mobile">
    ...
    <div class="box-product-finder container mb-5">
            ...
            <div class="box-product-finder-grid">
                @foreach($subCategories as $sub)
                <a href="{{ route('producsList',[preg_replace('/\s+/', '_', $sub->name),$sub->sub_pro_id])}}" class="box-product-finder-item d-flex">
                    <div class="m-auto">
                        @if($sub->image != null)
                        ...
                        @endif
                            ...
                            {{-- <div class="text-hover text-center">{!! iconv_substr(strip_tags($sub->content),0,90,'UTF-8') !!}...</div> --}}
                    </div>
                </a>
                @endforeach
            </div>

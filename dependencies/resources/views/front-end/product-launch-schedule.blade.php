    ...
    function loadproductLaunch(date){
        ...
        $.each(pro_launch_schedules, function(index,pro_launch){
         if(checkDateDate(pro_launch['monthdate'] ,date)){
            ...
            html += '<td class="text-middle-td"> <a href="{{config('app.url')}}/fie_doc/marketing_resources/'+pro_launch['file']+'" download > <div class="btn btn-ft w-100"> {{$staticContent['Downloads']}}</div></td>';
            html += '</tr>';
            ...
            mobilehtml += '</div>';
            mobilehtml +=  '<a href="{{config('app.url')}}/fie_doc/marketing_resources/'+pro_launch['file']+'" download >';

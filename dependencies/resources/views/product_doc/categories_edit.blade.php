...
<div class="content">
    <div class="block block-rounded block-bordered">
        ...
        <div class="block-content">
            <form action="{{route('updateDocCategories')}}" method="POST" enctype="multipart/form-data">
                {{csrf_field()}}
                <input  type="hidden" name="docCateId" value="{{$categorie[0]->id}}">
                <!-- Basic Elements -->
                <div class="row">
                    <div class="col-lg-12">
                            ...
                            name="title" value="{{$categorie[0]->title}}" placeholder="Enter title...">
                                </div>
                                <div class="form-group">
                                        <label class="d-block">Types <span class="req-fed">*</span></label>
                                       
                                        <div class="custom-control custom-radio custom-control-inline custom-control-primary">
                                                <input type="radio" class="custom-control-input" id="status-1" name="main_cate" value="1" {{ ($categorie[0]->main_cate_id == 1)?"checked":""}}>
                                                <label class="custom-control-label" for="status-1">Document</label>
                                            </div>
                                            <div class="custom-control custom-radio custom-control-inline custom-control-primary">
                                                <input type="radio" class="custom-control-input" id="status-2" name="main_cate" value="2" {{ ($categorie[0]->main_cate_id == 2)?"checked":""}}  >
                                                <label class="custom-control-label" for="status-2">Certificate</label>
                                            </div>
                                            <div class="custom-control custom-radio custom-control-inline custom-control-primary">
                                                    <input type="radio" class="custom-control-input" id="status-3" name="main_cate" value="3" {{ ($categorie[0]->main_cate_id == 3)?"checked":""}} >
                                ...
                                <div class="tab-pane {{($loop->iteration == 1)? "active" :""}}" id="btabs-alt-static-{{$item2->name}}" role="tabpanel">
                                    <div class="form-group">
                                        ...
                                        <input type="text"
                                            ...
                                            name="name[{{$item2->name}}]" value="{{isset($current->lable) ? $current->lable :''}}" placeholder="Enter name...">
                                    </div>
                                </div>
                                @endforeach
                            </div>

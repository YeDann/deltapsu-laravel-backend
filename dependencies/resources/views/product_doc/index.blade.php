    ...
    <div class="block block-rounded block-bordered">
        <div class="block-header block-header-default">
            ...
            <div class="block-options">
                ...
                <a href="{{route('createDocMutidoc')}}" class="btn btn-success">Create</a>
        ...
        <div class="block-content block-content-full">
            <table class="table table-bordered table-striped table-vcenter js-dataTable-full">
                ...
                <tbody>
                    @if(isset($docPros) and !empty($docPros))
                    @foreach ($docPros as $item)
                    <td class="text-center">{{$loop->iteration}}</td>
                    <td class="d-none d-sm-table-cell">{{$item['doc_id']}}</td>
                    <td class="d-none d-sm-table-cell">{{$item['doc_type']}}</td>
                    <td class="d-none d-sm-table-cell"> 
                        @foreach($item['prohas'] as $pro){{$pro->pro_code}} , @endforeach 
                    </td>
                    <td class="d-none d-sm-table-cell">{{$item['doc_name']}}</td>
...
<script>
    function getFiler(){
        ...
        if(value == 0){
            window.location = '{{route('product_doc')}}';
        }else{
            window.location = '{{route('docFilerBy')}}'+'/'+value;
        }

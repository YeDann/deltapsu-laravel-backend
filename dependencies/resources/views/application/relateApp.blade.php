    ...
    <div class="block block-rounded block-bordered">
        <div class="block-header block-header-default">
            <div class="block-title">
                    <form action="{{route('addRelatedSeries')}}" method="POST" enctype="multipart/form-data">
                        {{csrf_field()}}
                <div class="row"> 
                        <div class="col-lg-5"> 
                        ...
                        <option value="{{$se->series_id}}" >{{$se->title}}</option>
                                        @endforeach
        ...
        <div class="block-content block-content-full">
            <table class="table table-bordered table-striped table-vcenter">
                ...
                <tbody>
                    @if(isset($relatedApps) and !empty($relatedApps))
                    @foreach ($relatedApps as $item)
                   <tr class="odd order-list" data-id="{{$item->id}}">
                   <td class="text-center">{{$item->order_sq}}</td>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="block block-themed block-transparent mb-0">
                    ...
                    <form action="{{route('deleteRelatedSeries')}}" method="POST" >
                        {{csrf_field()}}
                        <input type="hidden" name="appId" value="{{$appId}}">

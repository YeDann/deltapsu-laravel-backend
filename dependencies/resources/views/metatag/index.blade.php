    ...
    <div class="block block-rounded block-bordered">
        <div class="block-header block-header-default">
            ...
            <div class="block-options">
                ...
                <div class="block-options-item">
                {{-- <a href="{{route('create_metaTag')}}" class="btn btn-success">Create</a> --}}
                </div>
            </div>
        </div>
        <div class="block-content block-content-full">
            <table class="table table-bordered table-striped table-vcenter js-dataTable-full">
                ...
                <tbody>
                    @if(isset($metatags) and !empty($metatags))
                    @foreach ($metatags as $item)
                    <td class="text-center">{{$loop->iteration}}</td>
                    <td class="d-none d-sm-table-cell">{{$item->page}}</td>
                    <td class="text-center">
                    <a href="{{route('edit_metaTag',$item->id)}}" class="btn btn-primary">Edit</a>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="block block-themed block-transparent mb-0">
                    ...
                    <form action="{{route('deleteMeta')}}" method="POST" >
                        {{csrf_field()}}
